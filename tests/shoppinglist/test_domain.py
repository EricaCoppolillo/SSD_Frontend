...
def test_name_format():
    wrong_values = ['','APP99LE','<script>alert()</script>','8APPLE','Ciao 90 30','A'*26]
    for value in wrong_values:
        with pytest.raises(ValidationError):
            Name(value)
...
def test_manufacturer_format():
    wrong_values = ['','A', 'APP99LE', '<script>alert()</script>', '8APPLE', 'Ciao 90 30', 'A' * 21]
    for value in wrong_values:
        with pytest.raises(ValidationError):
            Manufacturer(value)
...
def test_description_format():
    ...
    correct_values = ['Caratteristiche: CPU,GPU,Chipset', 'CPU=Snapdragon750', 'Il prodotto presenta dei "piccoli" segnali di deterioramento', '(Gigabyte)-Haorus', 'A' * 100]
    for value in correct_values:
...
def computers():
    return [
        ...
        Computer(Name('XS-500'), Manufacturer('Asus'), Price.create(16000), Quantity(1),Description("Questo prodotto è bellissimo")),
...
def smartphones():
    return [
        ...
        Smartphone(Name('Velvet'), Manufacturer('LG'), Price.create(590), Quantity(1),Description("Questo prodotto è bellissimo")),
        Smartphone(Name('Find X'), Manufacturer('Oppo'), Price.create(16000), Quantity(1),Description("Questo prodotto è bellissimo")),
...
def test_shoppinglist_remove_item(smartphones, computers):
    ...
    for smartphone in smartphones:
        shopping.add_smarphone(smartphone)
...
def test_shoppinglist_sort_by_manufacturer(smartphones, computers):
    ...
    shopping.add_smarphone(smartphones[0])
...
def test_shoppinglist_change_quantity(smartphones):
    ...
    assert  shopping.item(0).quantity == 1
